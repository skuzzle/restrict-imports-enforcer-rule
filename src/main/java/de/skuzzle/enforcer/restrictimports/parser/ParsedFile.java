...
public final class ParsedFile {
    ...
    private ParsedFile(Path path, String declaredPackage, String fqcn, Collection<ImportStatement> imports,
            boolean failedToParse, List<Annotation> annotations) {
        ...
        Preconditions.checkState(!failedToParse || !annotations.isEmpty(),
                "annotations must not be empty if failedToParse was true");
        this.path = path;
     ...
     * @param declaredPackage The package from the source file's <code>package</code>
     *            declaration
     ...
     * @since 2.2.0
     */
    public static ParsedFile successful(Path path, String declaredPackage, String fqcn,
            Collection<ImportStatement> imports) {
        return new ParsedFile(path, declaredPackage, fqcn, imports, false, Collections.emptyList());
    }

    /**
     * Creates a {@link ParsedFile} for the situation where the file could not be parsed
     * at all (technical problem occurred while parsing).
     *
    ...
    /**
     * Returns a new {@link ParsedFile} with identical information as this one but merging
     * this file's annotations with the provided annotations.
     *
    ...
    /**
     * Whether the file could not be parsed at all and we have no information about used
     * imports.
     *
    ...
    /**
     * A list of findings in this file apart from banned imports. Those findings will be
     * reported separately in the analysis report but will usually not fail the build.
     *
